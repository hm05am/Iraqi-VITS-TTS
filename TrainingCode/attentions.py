...
Includes multi-head attention with relative positional encoding
...
class LayerNorm(nn.Module):
    """Layer normalization that handles channels-first format."""
...
class MultiHeadAttention(nn.Module):
    ...
    def attention(self, query, key, value, mask=None):
        ...
        if self.window_size is not None:
            assert t_s == t_t, "Relative attention requires same length"
        ...
        if self.proximal_bias:
            assert t_s == t_t, "Proximal bias requires same length"
        ...
        if mask is not None:
            ...
            if self.block_length is not None:
                assert t_s == t_t, "Block length requires same length"
...
class Encoder(nn.Module):
    ...
    Used as the text encoder in VITS.
    """
