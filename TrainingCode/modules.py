"""
Neural network modules for VITS.
Includes WaveNet residual blocks, residual coupling layers,
and HiFi-GAN generator components.
"""
...
class DDSConv(nn.Module):
    ...
    Used in the stochastic duration predictor.
    """
...
class WN(nn.Module):
    """
    WaveNet residual block stack.
    Used in the posterior encoder and flow.
    """
...
class Log(nn.Module):
    """Logarithm transform for normalizing flows."""
    def forward(self, x, x_mask, reverse=False, **kwargs):
        if not reverse:
            y = torch.log(torch.clamp(x, 1e-5)) * x_mask
...
class ElementwiseAffine(nn.Module):
    """Element-wise affine transform for normalizing flows."""
...
class ConvFlow(nn.Module):
    """Convolution-based flow with neural spline coupling."""
