...
Single-speaker version adapted from jaywalnut310/vits.

Contains:
  - SynthesizerTrn: Full VITS model (text encoder, posterior encoder,
    HiFi-GAN decoder, normalizing flow, duration predictor)
  - MultiPeriodDiscriminator: Multi-period + multi-scale discriminator
"""
...
class StochasticDurationPredictor(nn.Module):
    ...
    Predicts a distribution over durations for each text token.
    """
    def __init__(self, in_channels, filter_channels, kernel_size, p_dropout,
                 n_flows=4, gin_channels=0):
        super().__init__()
        filter_channels = in_channels  # hidden size same as input
    ...
    def forward(self, x, x_mask, w=None, g=None, reverse=False, noise_scale=1.0):
        ...
        if not reverse:
            ...
            return nll + logq  # [batch]
        else:
            flows = list(reversed(self.flows))
            flows = flows[:-2] + [flows[-1]]  # remove initial flip
...
class DurationPredictor(nn.Module):
    ...
    Simple conv-stack that predicts log-duration for each text token.
    """
    def __init__(self, in_channels, filter_channels, kernel_size, p_dropout,
                 gin_channels=0):
        super().__init__()
...
class TextEncoder(nn.Module):
    """
    Text encoder using Transformer with relative positional encoding.
    Produces mean and log-variance for the prior distribution.
    """
    ...
    def forward(self, x, x_lengths):
        x = self.emb(x) * math.sqrt(self.hidden_channels)  # (b, t, d)
...
class ResidualCouplingBlock(nn.Module):
    """Stack of residual coupling layers for normalizing flow."""
...
class PosteriorEncoder(nn.Module):
    """
    Posterior encoder using WaveNet.
    Encodes mel/linear spectrogram to latent z.
    """
...
class Generator(nn.Module):
    """
    HiFi-GAN v1 generator (waveform decoder).
    Upsamples latent features to audio waveform.
    """
...
class MultiPeriodDiscriminator(nn.Module):
    """
    Multi-period discriminator combining period-based and scale-based sub-discriminators.
    """
    def __init__(self, use_spectral_norm=False):
        super().__init__()
        periods = [2, 3, 5, 7, 11]
...
class SynthesizerTrn(nn.Module):
    """
    Full VITS synthesizer for training.
    Single-speaker (no speaker embedding).
    
    Components:
      - TextEncoder: text → prior (m_p, logs_p)
      - PosteriorEncoder: spectrogram → posterior (z)
      - Generator (HiFi-GAN): z → waveform
      - ResidualCouplingBlock: normalizing flow (prior ↔ posterior)
      - StochasticDurationPredictor: predicts duration distribution
    ...
    def forward(self, x, x_lengths, y, y_lengths):
        ...
        Args:
            x: Text token IDs (batch, text_len)
            x_lengths: Text lengths (batch,)
            y: Linear spectrogram (batch, spec_channels, spec_len)
            y_lengths: Spectrogram lengths (batch,)
        """
    ...
    def infer(self, x, x_lengths, noise_scale=0.667, length_scale=1.0,
              noise_scale_w=0.8, max_len=None):
        """
        ...
        Args:
            x: Text token IDs (batch, text_len)
            x_lengths: Text lengths (batch,)
            noise_scale: Noise scale for prior sampling (lower = more deterministic)
    ...
    def voice_conversion(self, y, y_lengths, sid_src, sid_tgt):
        """Voice conversion (not used for single-speaker, kept for API compatibility)."""
        assert self.n_speakers > 0, "Voice conversion requires multi-speaker model"
